...
class LLMClient:
    ...
    def __init__(self):
        ...
        if self.provider == "local":
            print("💡 GEMINI_API_KEY / OPENAI_API_KEY not set. Running with Local Intelligent AI Engine.")
    ...
    def generate_completion(self, system_prompt: str, user_prompt: str, temperature: float = 0.2) -> str:
        ...
        if self.provider == "gemini" and self.gemini_client:
            ...
            for model_candidate in ["gemini-3.5-flash", "gemini-flash-latest", "gemini-3.1-flash-lite"]:
                try:
                    ...
                    print(f"⚠️ Gemini model '{model_candidate}' error: {e}. Trying next candidate...")
    ...
    def _local_fallback_completion(self, system_prompt: str, user_prompt: str) -> str:
        ...
        if "JSON" in system_prompt or "sentiment" in user_prompt.lower():
            ...
            if any(w in text for w in ["hot", "fresh", "delicious", "amazing", "great", "best", "excellent", "early"]):
                sentiment = "POSITIVE"
            elif any(w in text for w in ["cold", "horrible", "oily", "spilled", "stale", "bad", "rude", "delayed", "chewy"]):
                sentiment = "NEGATIVE"
            else:
                sentiment = "NEUTRAL"

            # Aspect detection
            if any(w in text for w in ["speed", "delayed", "early", "hour", "delivery", "tracking"]):
            ...
            elif any(w in text for w in ["flavour", "flavor", "taste", "delicious", "fresh", "stale", "oily", "chewy"]):
                aspect = "Food Quality"
            else:
                aspect = "Service"
        ...
        if "SQL" in system_prompt or "SELECT" in system_prompt:
            ...
            if "city" in prompt_lower and ("revenue" in prompt_lower or "gmv" in prompt_lower or "doanh thu" in prompt_lower):
            ...
            elif "top" in prompt_lower and ("restaurant" in prompt_lower or "nha hang" in prompt_lower or "nhà hàng" in prompt_lower):
            ...
            elif "delivery" in prompt_lower or "giao hang" in prompt_lower or "giao hàng" in prompt_lower:
            ...
            elif "user" in prompt_lower or "customer" in prompt_lower or "khach hang" in prompt_lower or "khách hàng" in prompt_lower:
